...
var theMap = window.theMap,
    snohomishCountyCoords = {
      MINX: 1245006.735392857,
      MAXX: 1622342.497,
   ...
   function private_smallCountyZoom(e) {
      ...
      if (e.clientX < clientRect.right - width) {

        return;
      }

      if (e.clientY < clientRect.bottom - height) {
  ...
  var private_mouseMove = function (e) {
      var left = e.clientX - this.leftt - this.offSetLeft,
          leftRatio = left/this.widthh;


      var top = e.clientY - this._top  - this.offSetTop,
          topRatio = top / this.heightt;
  ...
  var private_mouseUp = function (e) {
      ...
      var leftRatio = this.box.data.x / 1680,
          rightRatio = (this.box.data.x + this.box.data.width) / 1680;

      var topRatio = this.box.data.y / 927,
          bottomRatio = (this.box.data.y + this.box.data.height) / 927;

      var minX = (countyWidth* leftRatio) + snohomishCountyCoords.MINX,
  ...
  var smallCountySvgReCalc = function () {
    ...
    var mainMapHeightRatio = this.theMap.resizedMapHeight / this.theMap.resizedMapWidth,
        mainMapWidthRatio = this.theMap.resizedMapWidth / this.theMap.resizedMapHeight;

    var minX = ((this.theMap.presentMinX - snohomishCountyCoords.MINX) * widthPercent),
  ...
  var smallCountySvgResize = function () {
      var miniFooterTop = +(document.getElementById('mini_footer').getBoundingClientRect().height);

      var width = this.theMap.resizedMapWidth * 0.10,
          height = width * 0.5517857142857143; // 927/1680 is the viewBox coordinates, also the resolution of the monitor I was working on.

      var bottom = (this.theMap.resizedMapHeight * 0.05) + miniFooterTop,
  ...
  var init = function () {
    ...
    box: window.$('svg_rect'),
    smallCountySvg: smallCountySvgParent
  });
  ...
  var private_smallCountySvgCalculateStartPosition = function (arg_spX, arg_spY, arg_offsetWidth, arg_offsetHeight) {
      ...
      var countyWidth = 1680, countyHeight = 927;

      var xMultiplier = (snohomishCountyCoords.MAXX - snohomishCountyCoords.MINX) / countyWidth,
