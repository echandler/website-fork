theMap.infoSearch_module = function () {
    ...
    var private_getInfoFromSearchDiv = function () {
        var xmlRequest = undefined,
            ...
            apn = private_apnInput.value.replace(/[A-Za-z\s]/g, '').split(',').map(function (el) {
                        return "'"+ el +"'";
                    });
    ...
    var searchInfoAjaxOnload = function () {//{"a": "apn number goes here","x": lat,"y": lng,"m":"text message","i":"img url"}
        var parcelNumberRegex = /PARCEL_ID="(.*?)"/g,
            feature = undefined,
        ...
        if (/error/.test(private_searchInfoAjax.responseText)) {

            console.log(private_searchInfoAjax.responseText.match(/\<error.*?\<\/error/i))
        }
        ...
        while ((feature = featuresRegex.exec(private_searchInfoAjax.responseText)) !== null) {
            ...
            obj = {
                "a": parcelNumberRegex.exec(private_searchInfoAjax.responseText)[1],
            ...
            infoDiv.featureCount = featureCount && +featureCount[1];
    ...
    function private_makeResultsHTML(arg_infoObj) {
        ...
        theMap.markersArray.forEach(function (forEachArg_marker) {
            ...
            if (forEachArg_marker.data.apn == arg_infoObj.a) {
                ...
                containerDiv.markerBody = forEachArg_marker;
        ...
        containerDiv.addEventListener('mouseover', function () {
            ...
            if (this.markerBody && !this.markerBody.data.deleted) {

                this.markerBody.data.changeBgColorAndzIndex('white', 9999990);
            } else {

                private_searchForMarker(this);
            }
        });
    ...
    function private_makeSearchMarker(arg_infoObj, arg_infoDiv) {
        ...
        theMap.addMapLoadListener(
            ...
            function makeSearchMarker() {
                var noMarkerExistsYet = !(theMap.markersArray.some(function (arg_marker) {

                                            // Return true if there is already a marker with this APN.
                                            return arg_marker.data.apn == arg_infoObj.a;
                                         }));

                // If there is no marker yet then make a new one.
                if (!arg_infoDiv.markerBody || noMarkerExistsYet) {
    ...
    function private_searchPreviousResult(arg_result) {
        var previousResult = arg_result.previousElementSibling;
        ...
        if (previousResult) {
            ...
            arg_result.counterDiv.innerHTML = previousResult.id +' of '+ arg_result.featureCount;

            if (previousResult.markerBody && !previousResult.markerBody.data.deleted) {

                previousResult.markerBody.data.changeBgColorAndzIndex('white', 9999990);
            } else {

                private_searchForMarker(previousResult);
            }
    ...
    function private_searchNextResult(arg_result) {
        var nextResult = arg_result.nextElementSibling;
        ...
        if (nextResult) {
            ...
            arg_result.counterDiv.innerHTML = nextResult.id +' of '+ arg_result.featureCount;

            if (nextResult.markerBody && !nextResult.markerBody.data.deleted) {

                nextResult.markerBody.data.changeBgColorAndzIndex('white', 9999990);
            }   else {

                private_searchForMarker(nextResult);
            }
    ...
    function private_searchArrowUpClickEventHandler() {
        ...
        [].forEach.call($('search_results_div').children, function (elm) {
    ...
    var init = function () {
        ...
        private_searchContainer.addEventListener('mousedown', function (e) {
            ...
            var searchDocMouseMove = function (e) {
            ...
            if (e.target.nodeName === 'INPUT' || e.target.nodeName === 'BUTTON' || e.target.id == 'search_results_div') {
    ...
    function private_searchForMarker(arg_result) {

        theMap.markersArray.some(function (marker) {

            if (marker.data.apn == this.infoObj.a) {

                this.markerBody = marker;
