   ...
   var zoomInOut = function zoomInOut1(arg_e, arg_slider, arg_zoomBox) {
    ...
    var zoomInOut2 = function (arg_e, arg_slider, arg_zoomBox) {
        ...
        if (ratio === 2) {
            ...
            // Zooming in.
            x = 0 - (clientX - (this.dragDiv._left + this.currentMapImg._left) - (this.resizedMapWidth  / 2));
            y = 0 - (clientY - (this.dragDiv._top  + this.currentMapImg._top)  - (this.resizedMapHeight / 2));

            this.tempCSSTransformString = 'translate3d('+ x +'px,'+ y +'px, 0px) scale('+ ratio +')'+ this.tempCSSTransformString;
        } else if (ratio === 0.5) {
        ...
        this.currentMapImg.style[this.CSSTRANSFORM] =
            this.svgContainer.style[this.CSSTRANSFORM] =
                (this.baseCSSTransformValue + this.tempCSSTransformString);

        this.calculateMarkersPositions();

        if (!arg_slider && !arg_zoomBox) {// Only wait when person is zooming with mouse wheel.

            this.clearTimeoutt(this.zoomStartTimer);

            this.zoomStartTimer = this.setTimeoutt(function () {
    ...
    function zoomToMinExtent() { // Zooms to the minimum-ish extent that all the property markers will show up on.
        ...
        var zoomPowerKeys = Object.keys(theMap.ZOOM_POWER);
        ...
        for (var n = 0; n < zoomPowerKeys.length; ++n) {
            ...
            if (theMap.ZOOM_POWER[zoomPowerKeys[n]] < distance) {
                ...
                continue;
                 ...
                 document.getElementById('zoom_slider').style.top = theMap.sliderPosition +'px';
    ...
    var zoomToStatePlaneXY = function (arg_maxX, arg_minX, arg_maxY, arg_minY) {
        ...
        var x = (arg_maxX - arg_minX) / 2 + arg_minX,
            y = (arg_maxY - arg_minY) / 2 + arg_minY;

        var clientX = ((x - this.presentMinX) / xMultiplier),
...
function zoom(img) {
    ...
    function zoomInOut(arg_e) {// this === img.
        ...
        if (delta >= -120) {
            ...
            this.tempCSSTransformString = 'translate('+ (0-x) +'px,'+ (0-y) +'px) scale(2)' + this.tempCSSTransformString;

        } else if (delta <= -120) {

            x = (clientX  - (imgRect.width  / 2)) / 2;
            y = (clientY  - (imgRect.height / 2)) / 2;
            this.tempCSSTransformString = 'translate('+ x +'px,'+ y +'px) scale(0.5)' + this.tempCSSTransformString;

        }

        this.style.transform = this.tempCSSTransformString;
    }
    ...
    var pan = function (arg_e) {
        ...
        this.style.transform = 'translate('+ (left) +'px,'+ (top) +'px)' + this.tempCSSTransformString;
    }.bind(img);
    ...
    function mouseUp(arg_e) {// this === document.
        ...
        img.tempCSSTransformString = 'translate('+ x +'px,'+ y +'px)' + img.tempCSSTransformString;
    }
...
(function zoom() {function g(c) {document.removeEventListener("mousemove",h);document.removeEventListener("mouseup",g);a.tempCSSTransformString="translate("+(c.clientX-a.panX)+"px,"+(c.clientY-a.panY)+"px)"+a.tempCSSTransformString}var a=document.getElementById("testImg"),f=a.parentElement,e=a.getBoundingClientRect();f.style.display="block";f.style.width=e.width+"px";f.style.overflow="hidden";f.href="javascript: void(0);";a.onload="";a.src="http://i40.photobucket.com/albums/e212/chandleric/3mm%20leds/DSCF3371.jpg~original";
a.addEventListener("mousewheel",function (c) {var a=void 0,d=void 0,b=void 0,b=this.parentElement.getBoundingClientRect(),d=c.wheelDelta?c.wheelDelta:(a=window.event||c,-120*a.detail),a=c.clientX-b.left,b=c.clientY-b.top;c.preventDefault();120<=d?(d=a-e.width/2,b-=e.height/2,this.tempCSSTransformString="translate("+(0-d)+"px,"+(0-b)+"px) scale(2)"+this.tempCSSTransformString):-120>=d&&(d=(a-e.width/2)/2,b=(b-e.height/2)/2,this.tempCSSTransformString="translate("+d+"px,"+b+"px) scale(0.5)"+this.tempCSSTransformString);
this.style.transform=this.tempCSSTransformString});a.addEventListener("mousedown",function (a) {this.panX=a.clientX;this.panY=a.clientY;this.style.transition="";document.addEventListener("mousemove",h);document.addEventListener("mouseup",g)});a.tempCSSTransformString="";a.panX=0;a.panY=0;var h=function (a) {var e=a.clientX-this.panX,d=a.clientY-this.panY;a.preventDefault();this.style.transform="translate("+e+"px,"+d+"px)"+this.tempCSSTransformString}.bind(a)})()
...
function zoom(arg_img) {
    ...
    var pan = function (arg_e) {
        ...
        this.style.transform = 'translate('+ (arg_e.clientX - this.startPanX) +'px,'+ (arg_e.clientY - this.startanY) +'px)' + this.tempCSSTransformString;
    }.bind(newImg);
    ...
    function mouseUp(arg_e) {// this === document.
        ...
        newImg.tempCSSTransformString = 'translate('+ x +'px,'+ y +'px)' + newImg.tempCSSTransformString;
    }
