...
theMap.mapControl_module = function(){
    ...
    var mapLoad = function(){
        ...
        theMap.dragDiv.style.transition = '';
        theMap.svgContainer.setAttribute( 'class', '' );
        theMap.svgContainer.style[theMap.CSSTRANSFORM] = theMap.tempTransformText;
        ...
        theMap.calculateMarkerPosition(); // TODO: Markers don't need to be calculated every time the map loads.
        ...
        setTimeout( function(){ this.style.opacity = '1'; }.bind( this ), 20 );
        setTimeout( function(){
    ...
    var theMap_mouseDown = function ( e ){ // mouse down on theMap, either set a marker or drag the map.
        if ( e.which !== 1 ){ return; }
        if ( e.shiftKey ){ theMap.boxZoom_module.boxZoom_mouseDown( e ); return false; }
        e.preventDefault();
        mapImagesArray = [];
    ...
    var mapDragOnly = function( e ){
        this.dragDiv.style[this.CSSTRANSFORM] = 'translate('+ ( this.dragDiv._left + ( e.clientX - this.pan.mouseDownStartX ) ) +'px,'+ ( this.dragDiv._top + ( e.clientY - this.pan.mouseDownStartY ) ) +'px)';
    }.bind( theMap );
